...
import { hashSlice, IS_BLINK, IS_GECKO, IS_WEBKIT, IS_WORKER_SCOPE } from '../utils/helpers'
...
function queryLies({ scope, apiFunction, proto, obj, lieProps }: LiesConfig): LieResult {
...
const getNonFunctionToStringLies = (x: string[]) =>
	!x ? x : x.filter(x => !/object toString|toString incompatible proxy/.test(x)).length

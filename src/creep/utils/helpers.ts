...
const decryptUserAgent = ({ ua, os, isBrave }) => {
	...
	const safari =
		likeSafari && !firefox && !chromium && !edge && ua.match(/(version)\/(\d+)\.(\d|\.)+\s(mobile|safari)/i)
...
const getUserAgentRestored = ({ userAgent, userAgentData, fontPlatformVersion }) => {
	...
	const isGoogleChrome = /X11; CrOS/.test(userAgent) || !!(brands || []).find(x => x == 'Google Chrome')
	const versionNumber = +(/(\d+)\./.exec(platformVersion) || [])[1]
	const windowsFontVersion = (/8\.1|8|7/.exec(fontPlatformVersion) || [])[0]
	const windowsVersion = versionNumber >= 13 ? '11' : versionNumber == 0 ? windowsFontVersion || '7/8/8.1' : '10'
...
const getPromiseRaceFulfilled = async ({ promise, responseType, limit = 1000 }) => {

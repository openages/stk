...
import { getDiffs, html, patch, HTMLNote } from '../utils/html'
...
export default async function getClientRects() {
	try {
		...
		const emojiSet = emojiElems.reduce(
			(emojiSet, el, i) => {
		...
		const rectElems = DOC.getElementsByClassName('rects') as unknown as Array<Element>
...
export function clientRectsHTML(fp: any) {
	...
	const helpTitle = `Element.getClientRects()\nhash: ${hashMini(emojiSet)}\n${emojiSet
		.map((x: string, i: number) => (i && i % 6 == 0 ? `${x}\n` : x))
		.join('')}`

...
import { createTimer, hashSlice, logTestResult, performanceLogger, queueEvent, LowerEntropy } from '../utils/helpers'
import { getDiffs, modal, HTMLNote } from '../utils/html'

export const KnownAudio: Record<string, number[]> = {
	// Blink/WebKit
	[-20.538286209106445]: [
		124.0434488439787, 124.04344968475198, 124.04347527516074, 124.04347503720783, 124.04347657808103
	],
	[-20.538288116455078]: [
		124.04347518575378,
		124.04347527516074,
...
export default async function getOfflineAudioContext() {
	try {
		...
		const getRenderedBuffer = (context: OfflineAudioContext): Promise<AudioData | null> =>
			new Promise(resolve => {
				const analyser = context.createAnalyser()
				const oscillator = context.createOscillator()
				const dynamicsCompressor = context.createDynamicsCompressor()
		...
		const getSum = (arr?: Float32Array | number[]) =>
		...
		const known: Record<string, number[]> = {
			...
			'-20.538288116455078,164537.64805984497,502.5999283068122': [
				124.04347527516074, 124.04347518575378,
				// sus
				124.04347520065494, 124.04347523790784, 124.043475252809, 124.04347526025958,
		...
		const pattern = '' + [compressorGainReduction, floatFrequencyDataSum, floatTimeDomainDataSum]

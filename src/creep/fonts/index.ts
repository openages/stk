...
import { getRandomValues, lieProps, PHANTOM_DARKNESS } from '../lies'
...
import { count, html, patch, HTMLNote } from '../utils/html'
...
export default async function getFonts() {
	...
	try {
		...
		const doc =
			PHANTOM_DARKNESS && PHANTOM_DARKNESS.document && PHANTOM_DARKNESS.document.body
...
export function fontsHTML(fp) {
	...
	const {
		fonts: { $hash, fontFaceLoadFonts, platformVersion, apps, emojiSet, pixelSizeSystemSum, lied }
	} = fp
	...
	<div class="relative col-six${lied ? ' rejected' : ''}">
		...
		<div class="help" title="FontFace.load()">load (${fontFaceLoadFonts ? count(fontFaceLoadFonts) : '0'}/${
			'' + FONT_LIST.length
		}): ${

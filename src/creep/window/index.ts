...
import { createTimer, hashSlice, logTestResult, performanceLogger, IS_GECKO } from '../utils/helpers'
...
export function windowFeaturesHTML(fp) {
	...
	<div class="relative col-six">
		...
		<div>keys (${count(keys)}): ${
			keys && keys.length ? modal('creep-iframe-content-window-version', keys.join(', ')) : HTMLNote.BLOCKED
		}</div>

...
export default function getMaths() {
	try {
		...
		const fns = [
			...
			['pow', [n, -100], `pow(${n}, -100)`, 1.022089333584519e91, 1.0220893335845176e91, NaN, NaN],
			[
				'pow',
				[Math.PI, -100],
				'pow(Math.PI, -100)',
...
export function mathsHTML(fp) {
	...
	const results = Object.keys(data).map(key => {
		...
		return `
		${chrome ? '<span class="math-chromium">C</span>' : '<span class="math-blank-false">-</span>'}${
			firefox ? '<span class="math-firefox">F</span>' : '<span class="math-blank-false">-</span>'
		}${torBrowser ? '<span class="math-tor-browser">T</span>' : '<span class="math-blank-false">-</span>'}${
			safari ? '<span class="math-safari">S</span>' : '<span class="math-blank-false">-</span>'
		} ${key}`
	})
	...
	<div class="relative col-six${lied ? ' rejected' : ''}">
		...
		<div>results: ${!data ? HTMLNote.BLOCKED : modal('creep-maths', header + results.join('<br>'))}</div>

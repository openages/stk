...
import { documentLie, getPluginLies, lieProps } from '../lies'
import { gibberish, sendToTrash } from '../trash'
...
export default async function getNavigator(workerScope) {
	try {
		...
		await queueEvent(timer)
		return Promise.all([getUserAgentData(), getBluetoothAvailability(), getPermissions(), getWebGpu()])
			...
			${(userAgentData => {
				...
				return !userAgentData
					...
					<br>${windowsRelease || `${platform} ${platformVersion}`} ${
						architecture ? `${architecture}${bitness ? `_${bitness}` : ''}` : ''
					}
					${model ? `<br>${model}` : ''}
					${mobile ? '<br>mobile' : ''}
				`

...
export default async function getScreen(log = true) {
	try {
		...
		if (!firefoxWithHighDPR) {
			...
			const matchMediaLie = !matchMedia(`(device-width: ${width}px) and (device-height: ${height}px)`)
				.matches
			if (matchMediaLie) {
				lied = true
...
export function screenHTML(fp) {
	...
	const resizeHTML = ({ data, $hash, perf, paintScreen }) => {
		...
		const { deviceWidth: deviceInnerWidth, deviceHeight: deviceInnerHeight } = getDeviceDimensions(
			innerWidth,
			innerHeight
		)
		const toFix = (n, nFix) => {

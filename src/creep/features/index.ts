...
import { documentLie, PHANTOM_DARKNESS } from '../lies'
...
const getJSCoreFeatures = win => {
	...
	try {
		...
		const features = globalObjects.reduce((acc, name) => {
			...
			const uniques = [
				...new Set([...descriptorKeys, ...descriptorProtoKeys].filter(key => !ignore.includes(key)))
			]
...
export default async function getEngineFeatures({ cssComputed, navigatorComputed, windowFeaturesComputed }) {
	try {
...
export function featuresHTML(fp) {
	...
	const getModal = ({ id, engineMap, features, browser, report, userAgentVersion }) => {
		...
		const header =
			...
			<div>
			${diff && diff.added.length ? diff.added.map(key => `<div><span>${key}</span></div>`).join('') : ''}
			${

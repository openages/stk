...
import { hashMini, instanceId } from '../utils/crypto'
import { createTimer, hashSlice, logTestResult, performanceLogger, queueEvent, IS_BLINK } from '../utils/helpers'
import { modal, HTMLNote } from '../utils/html'
import { Platform } from './constants'
import getPlatformEstimate from './getPlatformEstimate'
import { getSystemFonts } from './getSystemFonts'

export default async function getHeadlessFeatures({ webgl, workerScope }) {
	try {
		...
		const data: Headless = {
			...
			headless: {
				...
				hasHeadlessUA:
					/HeadlessChrome/.test(navigator.userAgent) || /HeadlessChrome/.test(navigator.appVersion),
		...
		const likeHeadlessRating = +(
			(likeHeadlessKeys.filter(key => likeHeadless[key]).length / likeHeadlessKeys.length) *
			100
		).toFixed(0)
		const headlessRating = +(
			(headlessKeys.filter(key => headless[key]).length / headlessKeys.length) *
			100
		).toFixed(0)
		const stealthRating = +(
			(stealthKeys.filter(key => stealth[key]).length / stealthKeys.length) *
			100
		).toFixed(0)
...
export function headlessFeaturesHTML(fp) {
	...
	<div class="relative col-six">
		...
		<div class="like-headless-rating">${'' + likeHeadlessRating}% like headless: ${modal(
			'creep-like-headless',
